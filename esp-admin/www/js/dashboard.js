...
function dashboard() {
  return {
    ...
    drawSensors(ch, index) {
      ...
      switch (type.name) {
        case "PZEM004": count = 4;
          break;
        case "PZEM004t": count = 7;
          break;      
        default:
          break;
      }
      ...
      for(var i = 0; i < count; i++) {
        ...
        html +=       '</svg>';
        html +=     '</div>';
        html +=     '<p class="ml-16 text-sm font-medium text-gray-500 truncate"> Channel ' + this.channelTitle(ch, index) + '</p>';
        html +=   '</dt>';
        html +=   '<dd class="flex flex-col">';
        html +=     '<div class="w-full">';
        html +=       this.drawSensor(ch, i);
        html +=     '</div>';
